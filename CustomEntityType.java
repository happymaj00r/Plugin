...
public class CustomEntityType {
    ...
    public static boolean addRandomSpawn(SpawnData data, MobMeta meta, Biome... biomes) {
        ...
        if ((field = meta.getField()) == null) {
        	System.out.println("FEEEEEEEEEEEEEEEEEEEEHLEEEEEEEEEEEEEEEEEEEEEEEEEER!!!!!!");
            return false;
        }
        try {
    ...
    public static void registerEntity(Type type, Class<? extends Entity> customClass, boolean override) {
    	CustomEntityType.registerEntity(type.getName(), type, customClass, override);
    }
    ...
    public static void registerEntity(String name, Type type, Class<? extends Entity> customClass, boolean override) {
        ...
        try {
            field.setAccessible(true);
            for (BiomeBase base : CustomEntityType.BIOMES) {
                List<BiomeMeta> list = (List<BiomeMeta>) field.get(base);
                for (BiomeMeta meta : list) {
                    if (meta.b == type.getNMSClass()) {
                        meta.b = (Class<? extends EntityInsentient>) customClass;
    ...
    public enum MobMeta {
        MONSTER(CustomEntityType.META_LIST_MONSTER),
        CREATURE(CustomEntityType.META_LIST_CREATURE),
        WATER_CREATURE(CustomEntityType.META_LIST_WATER_CREATURE),
        AMBIENT(CustomEntityType.META_LIST_AMBIENT),
        UNDEFINED(null);
    ...
    static {
        ...
        MINECRAFT_SERVER = CustomEntityType.CRAFTBUKKIT_SERVER.getServer();
        BIOMES = new BiomeBase[BiomeBase.i.a()];
        Iterator<BiomeBase> iterator = BiomeBase.i.iterator();
        int index = 0;
        while (iterator.hasNext()) {
        	CustomEntityType.BIOMES[index++] = iterator.next();
        }
    }
}